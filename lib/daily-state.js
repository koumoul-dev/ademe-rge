...
exports.readDailyState = async (ftp, dir, folder, day, qualifDomaine, ftpBasepath, log) => {
  ...
  const state = {}
  const qualificationsLines = parse(decodeCp1252(files.qualifications), parserOpts)
  const qualifications = qualificationsLines.reduce((a, q) => { a[q[0]] = q[1]; return a }, {})
  const entreprisesLines = parse(decodeCp1252(files.entreprises), parserOpts)
  const entreprises = entreprisesLines.reduce((a, e) => { a[e[0]] = e; return a }, {})
  const liens = parse(decodeCp1252(files.liens), parserOpts)
  ...
  for (const lien of liens) {
    ...
    const entrepriseIdOrganisme = entreprise[0]
    state[`${entrepriseIdOrganisme}-${data.code_qualification}`] = data
  }

  if (entreprisesMiss.size) log.error(`${folder} - ${day} - entreprises manquantes : ${[...entreprisesMiss].join(', ')}`)
  if (domainesMiss.size) log.error(`${folder} - ${day} - domaines manquants pour ces codes qualification : ${[...domainesMiss].join(', ')}`)
  if (qualifMiss.size) log.error(`${folder} - ${day} - qualifications manquantes : ${[...qualifMiss].join(', ')}`)
  return state
}
