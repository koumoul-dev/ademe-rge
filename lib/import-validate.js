...
const validateQualificationsLine = (line, i, errors, context) => {
  ...
  if (!line[0]) errors.push(prefix + 'le champ "code" est obligatoire')
    ...
    if (!line[0].replace(/-/g, '').match(/^[A-Za-z0-9]{0,10}$/)) errors.push(prefix + 'le champ "code" n\'a pas le format attendu - ' + line[0])
  }
...
const validateLiensLine = (line, i, errors, context) => {
  ...
  else if (!line[1].replace(/-/g, '').match(/^[A-Za-z0-9]{0,10}$/)) errors.push(prefix + 'le champ "qualification_code" n\'a pas le format attendu - ' + line[1])
