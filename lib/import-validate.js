...
const { decodeCp1252 } = require('./decode')
...
// dates in the CSV files are concatenated YYYYMMDD ; we need them to be both
// 8 digits AND a real calendar date, otherwise day2date() builds an invalid
// "date" string (e.g. "0000-00-00", "2026-06-31") that data-fair rejects on _bulk_lines
const isValidDay = (str) => /^[0-9]{8}$/.test(str) && moment(str, 'YYYYMMDD', true).isValid()

const validateFormat = async (filePath, nbCols, validateLine, context, prefix) => {
  const errors = []
  // TODO: a way of checking ANSI/windows-1252 encoding ?
  try {
    const lines = parse(decodeCp1252(await fs.readFile(filePath)), parserOpts)
    const badNbCols = []
    const badChars = []
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.length !== nbCols) {
        ...
        if (line.some(item => item.includes(';') || item.includes('\n') || item.includes('"'))) badChars.push(i + 1)
    ...
    if (badChars.length) {
      errors.unshift(`${prefix}${badChars.length}/${lines.length} lignes contiennent des caractères interdits (lignes ${badChars.length > 5 ? badChars.slice(0, 5).join(', ') + ', ...' : badChars.join(', ')})`)
    }
  } catch (err) {
...
exports.validateLiensLine = (line, i, errors, context) => {
  ...
  else if (!isValidDay(line[2])) errors.push(prefix + 'le champ "date début" n\'est pas une date valide - ' + line[2])
  // date fin
  ...
  else if (!isValidDay(line[3])) errors.push(prefix + 'le champ "date fin" n\'est pas une date valide - ' + line[3])
  // url qualification
...
exports.downloadAndValidate = async (ftp, dir, folder, files, ftpBasePath, log) => {
  ...
  if (!files.includes('liens.csv')) {
    ...
    const formatErrors = await validateFormat(path.join(importFolder, 'liens.csv'), 7, exports.validateLiensLine, context, 'liens.csv - ')
    for (const error of formatErrors) await log.error(`${folder} - ${error}`)
    errors = errors.concat(formatErrors)
  }
  return errors
