...
const validateFormat = async (filePath, nbCols, validateLine, context) => {
  ...
  try {
    ...
    if (badNbCols.length) {
      errors.unshift(`${badNbCols.length}/${lines.length} lignes n'ont pas les ${nbCols} éléments attendus (lignes ${badNbCols.length > 5 ? badNbCols.slice(0, 5).join(', ') + ', ...' : badNbCols.join(', ')})`)
    }
    if (badChars.length) {
      errors.unshift(`${badChars.length}/${lines.length} lignes contiennents des caractères interdits (lignes ${badChars.length > 5 ? badChars.slice(0, 5).join(', ') + ', ...' : badChars.join(', ')})`)
    }
  } catch (err) {
...
exports.downloadAndValidate = async (ftp, dir, folder, files, log) => {
  ...
  if (!files.includes('entreprises.csv')) {
    ...
    const formatErrors = await validateFormat(path.join(importFolder, 'entreprises.csv'), 13, validateEntrepriseLine, context)
  ...
  if (!files.includes('qualifications.csv')) {
    ...
    const formatErrors = await validateFormat(path.join(importFolder, 'qualifications.csv'), 2, validateQualificationsLine, context)
    for (const error of formatErrors) await log.error(error)
    errors = errors.concat(formatErrors)
  }
  if (!files.includes('liens.csv')) {
    ...
    const formatErrors = await validateFormat(path.join(importFolder, 'liens.csv'), 7, validateLiensLine, context)
    for (const error of formatErrors) await log.error(error)
    errors = errors.concat(formatErrors)
  }
...
exports.moveToFtp = async (ftp, dir, folder, error, log) => {
  ...
  for (const file of ['entreprises', 'qualifications', 'liens']) {
    ...
    if (await fs.pathExists(filePath)) {
      ...
      await ftp.put(`${file}.csv`, path.join(ftpPath, ftpDir, `${day}-${file}.csv`))
