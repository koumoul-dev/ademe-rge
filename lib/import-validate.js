...
const { parseNumber } = require('./format')

const validateFormat = async (filePath, nbCols, validateLine, context) => {
  const errors = []
  ...
  try {
    ...
    if (badNbCols.length) {
      errors.unshift(`${badNbCols.length}/${lines.length} lignes n'ont pas les ${nbCols} éléments attendus (${badNbCols.length > 3 ? badNbCols.slice(0, 3).join(', ') + ', ...' : badNbCols.join(', ')})`)
    }
    if (badChars.length) {
      errors.unshift(`${badChars.length}/${lines.length} lignes contiennents des caractères interdits (${badChars.length > 3 ? badChars.slice(0, 3).join(', ') + ', ...' : badChars.join(', ')})`)
    }
  } catch (err) {
...
const validateEntrepriseLine = (line, i, errors, context) => {
  const prefix = `entreprises.csv ligne ${i} - `
...
const validateQualificationsLine = (line, i, errors, context) => {
  const prefix = `qualifications.csv ligne ${i} - `
  // code
...
const validateLiensLine = (line, i, errors, context) => {
  const prefix = `liens.csv ligne ${i} - `
  // entreprise_id_organisme
  if (!line[0]) errors.push(prefix + 'le champ "entreprise_id_organisme" est obligatoire')
...
exports.downloadAndValidate = async (ftp, dir, folder, day, tar, log) => {
  ...
  await log.info('téléchargement du fichier ' + tar)
  await fs.emptyDir(importFolder)
  const filePath = path.join(importFolder, tar)
  // creating empty file before streaming seems to fix some weird bugs with NFS
  await fs.ensureFile(filePath + '.tmp')
  await pump(await ftp.get(ftpPath + tar), fs.createWriteStream(filePath + '.tmp'))
  ...
  await log.info('extraction du contenu du fichier ' + tar)
  execSync(`tar -xvzf ${tar}`, { cwd: importFolder })

  const files = (await fs.readdir(importFolder)).filter(f => f !== tar)
  const context = { idEntreprises: new Set(), codeQualifs: new Set() }
  if (!files.includes(`${day}-entreprises.csv`)) {
    await errors.push(`l'archive ne contient pas de fichier ${day}-entreprises.csv`)
  } else {
    await log.info('vérification du format du fichier entreprises.csv')
    const formatErrors = await validateFormat(path.join(importFolder, `${day}-entreprises.csv`), 13, validateEntrepriseLine, context)
    for (const error of formatErrors) await log.error(error)
    errors = errors.concat(formatErrors)
  }
  if (!files.includes(`${day}-qualifications.csv`)) {
    await errors.push(`l'archive ne contient pas de fichier ${day}-qualifications.csv`)
  } else {
    await log.info('vérification du format du fichier qualifications.csv')
    const formatErrors = await validateFormat(path.join(importFolder, `${day}-qualifications.csv`), 2, validateQualificationsLine, context)
    for (const error of formatErrors) await log.error(error)
    errors = errors.concat(formatErrors)
  }
  if (!files.includes(`${day}-liens.csv`)) {
    await errors.push(`l'archive ne contient pas de fichier ${day}-liens.csv`)
  } else {
    await log.info('vérification du format du fichier liens.csv')
    const formatErrors = await validateFormat(path.join(importFolder, `${day}-liens.csv`), 7, validateLiensLine, context)
    for (const error of formatErrors) await log.error(error)
    errors = errors.concat(formatErrors)
  }
...
exports.moveToFtp = async (ftp, dir, folder, day, error, tar, log) => {
  const importFolder = path.join(dir, folder, 'import')
  const ftpDir = path.join(folder, error ? '/erreur' : '/archive')
  for (const file of ['entreprises', 'qualifications', 'liens']) {
    const filePath = path.join(importFolder, `${day}-${file}.csv`)
    if (await fs.pathExists(filePath)) {
      await log.info(`envoi de ${file}.csv -> ` + path.join(ftpDir, `${day}-${file}.csv`))
      await ftp.put(file, path.join(ftpPath, ftpDir, `${day}-${file}.csv`))
    }
  }

  await log.info(`suppression de l'archive ${tar}`)
  await ftp.delete(path.join(ftpPath, tar))
}
