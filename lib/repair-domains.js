module.exports = async (axios, dataset, qualifDomaine, log) => {
  await log.step('Finalisation')
  await log.info('Récupération de la liste des domaines inconnus')
  const missing = (await axios.get(`api/v1/datasets/${dataset.id}/values_agg`, { params: { field: 'code_qualification', domaine_in: 'Inconnu', agg_size: 1000 } })).data.aggs
  if (missing.length) {
